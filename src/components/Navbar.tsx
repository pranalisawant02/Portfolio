  ...
  return (
    <>
      ...
      >
        ...
        <div className="absolute bottom-0 left-0 h-[1px] bg-gradient-to-r from-primary via-secondary to-accent transition-all duration-150 opacity-50" style={{ width: `${scrollProgress}%` }} />
      </motion.nav>
      ...
      <AnimatePresence>
        {mobileOpen && (
          ...
          >
            <div className="absolute inset-0 bg-dark/98 backdrop-blur-2xl" onClick={() => setMobileOpen(false)} />
            <motion.div className="relative flex flex-col items-center justify-center h-full gap-6 pt-20">
              ...
              >
                ...
                <p className="mt-3 text-xs tracking-[0.25em] uppercase text-primary">AI/ML Engineer</p>
              </motion.div>
              {navLinks.map((link, i) => (
                <motion.a
                  key={link.name}
                  ...
                  className={`text-2xl font-semibold transition-colors ${
                    activeSection === link.href.replace('#', '') ? 'gradient-text-cyan' : 'text-muted hover:text-light'
                  }`}
                >
                  {link.name}
                </motion.a>
